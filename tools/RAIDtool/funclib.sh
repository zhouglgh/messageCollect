...
function get_storage_info()
{
    ...
    if [ "$(uname -m)" == "x86_64" ]; then
        TOOLS="./MegaRAID/storcli/storcli64"
        megacli="./MegaCli/MegaCli64"
    else
        TOOLS="./MegaRAID/storcli/storcli"
        megacli="./MegaCli/MegaCli"
    fi
...
function get_lsi2308_log()
{
    local sas2ircu="./2308/sas2ircu"
    local sas2flash="./2308/sas2flash"
...
function get_lsi3008_log()
{
    local sas3ircu="./3008/sas3ircu"
    local sas3flash="./3008/sas3flash"
    local log_file="output/sashbalog.txt"
...
function get_megaraid_log()
{
    ...
    run_cmd "$megacli -LDBI -ProgDsply -Lall -aALL " "output/MegaCliLDBIProgDsply"
...
function get_pmc_info()
{
   local arcconf=""
    if [ "$(uname -m)" == "x86_64" ]; then
        arcconf="./Arcconf-Linux/arcconf-x64"
    else
        arcconf="./Arcconf-Linux/arcconf-x86"
    fi
